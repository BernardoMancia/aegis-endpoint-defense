    ...
    if active_item:
        ...
        <div class="flex flex-col md:flex-row h-screen overflow-hidden">
            ...
            <main class="flex-1 p-3 md:p-8 mt-12 md:mt-0 overflow-y-auto custom-scrollbar">
                {{% if session.get('soc_role') == 'viewer' %}}
                <div class="mb-6 hyper-glass border-l-4 border-l-amber-500 bg-amber-500/10 px-6 py-4 flex items-center gap-4 animate-pulse">
                    <i data-lucide="eye" class="text-amber-500 w-6 h-6"></i>
                    <div>
                        <h4 class="text-amber-500 font-bold text-sm tracking-tight uppercase">Modo de Apenas Leitura</h4>
                        <p class="text-amber-500/70 text-[10px] font-medium">Você tem permissão apenas para visualizar dados. Alterações e ações do SOAR estão desabilitadas para o seu nível de acesso.</p>
                    </div>
                </div>
                {{% endif %}}
                {content}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="role" class="aegis-input">
                                    <option value="viewer" {% if target.role == 'viewer' %}selected{% endif %}>Viewer (Apenas Leitura)</option>
                                    <option value="operator" {% if target.role == 'operator' %}selected{% endif %}>Operator (Resposta Básica)</option>
                                    <option value="analyst" {% if target.role == 'analyst' %}selected{% endif %}>Analyst (SOC Forense)</option>
                                    <option value="admin" {% if target.role == 'admin' %}selected{% endif %}>Admin (Gestão Geral)</option>
                                    <option value="superadmin" {% if target.role == 'superadmin' %}selected{% endif %}>Super Admin (Controle Total)</option>
                                </select>
                            </div>
                             <div class="space-y-2">
                    ...
                    <div class="hyper-glass p-8 border-rose-500/10">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <button id="det-btn-isolate" onclick="toggleIsolate()" class="w-full p-4 rounded-xl border border-white/5 bg-white/5 hover:bg-rose-500/10 hover:border-rose-500/30 transition-all flex items-center gap-4 group">
                                <div id="det-iso-icon-bg" class="p-2 rounded-lg bg-rose-500/10 text-rose-500 group-hover:scale-110 transition-transform"><i id="det-iso-icon" data-lucide="lock" class="w-5 h-5"></i></div>
                                <div class="text-left">
                                    <div id="det-iso-text" class="text-xs font-black text-white uppercase">Host Isolation</div>
                ...
                async function fetchFullDetail() {
                    try {
                        ...
                        if(res.ok) {
                            ...
                            const isoText = document.getElementById('det-iso-text');
                            const isoIcon = document.getElementById('det-iso-icon');
                            const isoIconBg = document.getElementById('det-iso-icon-bg');
                            
                            if(ag.isolation_active) {
                                isoBtn.onclick = () => runSoarAction('unisolate');
                                if(isoText) isoText.innerText = "Remove Isolation";
                                if(isoIconBg) isoIconBg.className = "p-2 rounded-lg bg-emerald-500/10 text-emerald-500 group-hover:scale-110 transition-transform";
                                if(isoIcon) isoIcon.setAttribute('data-lucide', 'shield-off');
                            } else {
                                isoBtn.onclick = () => runSoarAction('isolate');
                                if(isoText) isoText.innerText = "Host Isolation";
                                if(isoIconBg) isoIconBg.className = "p-2 rounded-lg bg-rose-500/10 text-rose-500 group-hover:scale-110 transition-transform";
                                if(isoIcon) isoIcon.setAttribute('data-lucide', 'lock');
                            }
