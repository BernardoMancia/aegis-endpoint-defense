...
def rebuild_dashboard():
    dashboard_html = """
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div class="hyper-glass p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div class="p-2 bg-sky-500/10 rounded-lg text-sky-500"><i data-lucide="monitor" class="w-5 h-5"></i></div>
                ...
                <div class="hyper-glass p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div class="p-2 bg-emerald-500/10 rounded-lg text-emerald-500"><i data-lucide="wifi" class="w-5 h-5"></i></div>
                ...
                <div class="hyper-glass p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div class="p-2 bg-rose-500/10 rounded-lg text-rose-400"><i data-lucide="shield-alert" class="w-5 h-5"></i></div>
                ...
                <div class="hyper-glass p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div class="p-2 bg-indigo-500/10 rounded-lg text-indigo-400"><i data-lucide="activity" class="w-5 h-5"></i></div>
            ...
            <!-- Slide Panel -->
            <div id="agent-panel" class="fixed top-0 right-0 w-[500px] h-screen bg-slate-900/95 backdrop-blur-2xl border-l border-slate-800 z-[100] translate-x-full transition-transform duration-500 ease-in-out shadow-2xl flex flex-col">
                <div class="p-6 border-b border-slate-800 flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold" id="panel-hostname">Hostname</h2>
                        <p class="text-slate-500 text-xs font-mono" id="panel-agent-id">ID: --</p>
                    </div>
                    <button onclick="closeAgentPanel()" class="p-2 hover:bg-white/10 rounded-lg transition-colors text-slate-500 hover:text-white">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-8">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 rounded-xl bg-white/5 border border-white/5">
                            <p class="text-[10px] uppercase text-slate-500 font-bold mb-1">Caminho do Agente</p>
                            <p class="text-xs truncate font-mono" id="panel-path">--</p>
                        </div>
                        <div class="p-4 rounded-xl bg-white/5 border border-white/5">
                            <p class="text-[10px] uppercase text-slate-500 font-bold mb-1">Platforma</p>
                            <p class="text-xs font-mono" id="panel-platform">--</p>
                        </div>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold mb-3 flex items-center gap-2"><i data-lucide="terminal" class="w-4 h-4 text-sky-400"></i> Terminal Reverso</h3>
                        <div class="rounded-xl bg-black/50 border border-slate-800 p-4 font-mono text-[11px] h-64 overflow-y-auto mb-3" id="terminal-output">
                            <span class="text-slate-600">Selecione um agente para estabelecer conexão...</span>
                        </div>
                        <div class="flex gap-2">
                            <input type="text" id="terminal-input" class="aegis-input flex-1 !py-2 !text-xs font-mono" placeholder="Digite um comando..." onkeydown="if(event.key==='Enter')sendTerminalCommand()">
                            <button onclick="sendTerminalCommand()" class="aegis-btn-primary !py-2 !px-4"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <h3 class="text-sm font-bold flex items-center gap-2"><i data-lucide="zap" class="w-4 h-4 text-amber-500"></i> Orquestração SOC</h3>
                        <div class="grid grid-cols-2 gap-3">
                            <button onclick="soarAction('screenshot')" class="aegis-btn-secondary !text-xs flex gap-2"><i data-lucide="monitor" class="w-4 h-4 text-sky-400"></i> Screenshot</button>
                            <button onclick="soarAction('force_scan_vulns')" class="aegis-btn-secondary !text-xs flex gap-2"><i data-lucide="search" class="w-4 h-4 text-emerald-400"></i> Scan Vulns</button>
                            <button onclick="soarAction('force_logs')" class="aegis-btn-secondary !text-xs flex gap-2"><i data-lucide="list" class="w-4 h-4 text-purple-400"></i> Get Logs</button>
                            <button onclick="soarAction('force_scan_network')" class="aegis-btn-secondary !text-xs flex gap-2"><i data-lucide="network" class="w-4 h-4 text-indigo-400"></i> Net Scan</button>
                        </div>
                        <div class="pt-4 border-t border-slate-800">
                            <h4 class="text-[10px] font-bold text-rose-500 uppercase mb-3 text-center">Protocolos de Contenção</h4>
                            <div class="grid grid-cols-2 gap-3">
                                <button onclick="soarAction('isolate')" class="p-3 rounded-lg border border-rose-500/30 bg-rose-500/10 text-rose-500 text-xs font-bold hover:bg-rose-500/20 transition-all flex items-center justify-center gap-2">
                                    <i data-lucide="lock" class="w-4 h-4"></i> ISOLAR
                                </button>
                                <button onclick="soarAction('uninstall')" class="p-3 rounded-lg border border-slate-700 bg-slate-800 text-slate-300 text-xs font-bold hover:border-rose-500/50 hover:text-rose-400 transition-all flex items-center justify-center gap-2">
            ...
            <script>
                ...
                function closeAgentPanel() { document.getElementById('agent-panel').classList.add('translate-x-full'); selectedAgentId = null; }
                async function openAgentPanel(agentId) {
                    selectedAgentId = agentId;
                    const agent = currentAgents.find(a => a.id === agentId);
                    if(!agent) return;
                    document.getElementById('panel-hostname').innerText = agent.hostname;
                    document.getElementById('panel-agent-id').innerText = `ID: ${agent.id}`;
                    document.getElementById('panel-platform').innerText = agent.platform;
                    document.getElementById('panel-path').innerText = agent.agent_path || '---';
                    document.getElementById('agent-panel').classList.remove('translate-x-full');
                    document.getElementById('terminal-output').innerHTML = '<span class="text-emerald-500">Conexão estabelecida com shell remoto...</span>';
                }
                ...
                function renderAgents() {
                    ...
                    grid.innerHTML = currentAgents.map(ag => `
                        <div onclick="openAgentPanel('${ag.id}')" class="hyper-glass p-5 cursor-pointer border-l-4 ${ag.status === 'online' ? 'border-l-emerald-500' : 'border-l-slate-700'} group hover:scale-[1.02] transition-all">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full ${ag.status === 'online' ? 'bg-emerald-500' : 'bg-slate-500'}"></span>
                                    <span class="font-bold text-sm">${ag.hostname}</span>
                                </div>
                                <i data-lucide="${ag.platform.includes('win') ? 'server' : 'smartphone'}" class="w-4 h-4 text-slate-500 group-hover:text-sky-400 transition-colors"></i>
                            </div>
                            <p class="text-[10px] text-slate-500 font-mono">${ag.last_ip || '0.0.0.0'} • ${ag.os_version || 'N/A'}</p>
                            ${ag.is_isolated ? '<span class="mt-2 inline-block px-2 py-0.5 rounded text-[9px] font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30">ISOLADO</span>' : ''}
                        </div>
                    `).join('');
                    lucide.createIcons();
                }
                ...
                function renderIncidents(incidents) {
                    ...
                    feed.innerHTML = incidents.map(inc => `
                        <div class="p-4 rounded-xl bg-white/5 border border-white/5 flex gap-4 transition-all hover:bg-white/[0.08]">
                            <div class="mt-1 ${inc.severity === 'CRITICAL' ? 'text-rose-500' : 'text-amber-500'}">
                                <i data-lucide="alert-triangle" class="w-5 h-5"></i>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold">${inc.title}</h4>
                                <p class="text-[11px] text-slate-400 mt-1 line-clamp-2">${inc.description}</p>
                                <div class="flex gap-2 mt-2 items-center">
                                    <span class="text-[9px] font-bold px-1.5 py-0.5 rounded ${inc.severity === 'CRITICAL' ? 'bg-rose-500/20 text-rose-500' : 'bg-amber-500/20 text-amber-500'}">${inc.severity}</span>
                                    <span class="text-[9px] text-slate-500">${new Date(inc.created_at).toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    `).join('');
                ...
                function showToast(msg, type='info') {
                    ...
                    let icon = 'info';
                    if(type === 'error') { toast.classList.add('border-l-4', 'border-l-rose-500'); icon = 'alert-octagon'; }
                    if(type === 'success') { toast.classList.add('border-l-4', 'border-l-emerald-500'); icon = 'check-circle'; }
                ...
                async function soarAction(action) {
                    ...
                    showToast(`Despachando ação: ${action}...`);
                    try {
                        const res = await fetch(`/control/${selectedAgentId}/soar`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action}) });
                        const data = await res.json();
                        if(data.status === 'ok') showToast("Comando enviado com sucesso!", "success");
                        else showToast(data.message || "Erro ao enviar.", "error");
                ...
                async function sendTerminalCommand() {
                    ...
                    out.innerHTML += `<div class="text-sky-400 mt-2">❯ ${cmd}</div>`;
                    input.value = '';
                    try {
                        const res = await fetch(`/control/${selectedAgentId}/shell`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({command: cmd}) });
                        const data = await res.json();
                        if(data.status === 'queued') out.innerHTML += `<div class="text-slate-500">Comando enfileirado...</div>`;
                        else out.innerHTML += `<div class="text-rose-400">Falha: ${data.message}</div>`;
                ...
                async function sendAIChat() {
                    ...
                    messages.innerHTML += `<div class="bg-indigo-500/10 p-3 rounded-xl ml-4 text-sm text-slate-200 border border-indigo-500/20">${msg}</div>`;
                    input.value = '';
                    ...
                    try {
                        ...
                        messages.innerHTML += `<div class="bg-white/5 p-3 rounded-xl mr-4 text-sm text-sky-200 border border-white/5"><strong>Aegis AI:</strong> ${d.reply || 'Processando análise...'}</div>`;
                        messages.scrollTop = messages.scrollHeight;
